#ifndef CCYSTL_UNORDERED_SET_H_
#define CCYSTL_UNORDERED_SET_H_
...
//
// 异常保证：
// ccystl::unordered_set<Key> / ccystl::unordered_multiset<Key> 满足基本异常保证，对以下等函数做强异常安全保证：
//   * emplace
//   * emplace_hint
//   * insert
...
namespace ccystl {
    ...
    // 参数一代表键值类型，参数二代表哈希函数，缺省使用 ccystl::hash，
    // 参数三代表键值比较方式，缺省使用 ccystl::equal_to
    template <class Key, class Hash = ccystl::hash<Key>, class KeyEqual = ccystl::equal_to<Key>>
    class unordered_set {
    private:
        // 使用 hashtable 作为底层机制
