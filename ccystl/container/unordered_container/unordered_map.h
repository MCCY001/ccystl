#ifndef CCYSTL_UNORDERED_MAP_H_
#define CCYSTL_UNORDERED_MAP_H_

// 这个头文件包含模板类 unordered_map
// 功能与用法与 map 和 multimap 类似，不同的是使用 hashtable 作为底层实现机制，容器内的元素不会自动排序

// notes:
//
// 异常保证：
// ccystl::unordered_map<Key, T> / ccystl::unordered_multimap<Key, T> 满足基本异常保证，对以下等函数做强异常安全保证：
//   * emplace
//   * emplace_hint
//   * insert
...
namespace ccystl {
    ...
    // 参数一代表键值类型，参数二代表实值类型，参数三代表哈希函数，缺省使用 ccystl::hash
    // 参数四代表键值比较方式，缺省使用 ccystl::equal_to
    template <class Key, class T, class Hash = ccystl::hash<Key>, class KeyEqual = ccystl::equal_to<Key>>
    class unordered_map {
    private:
        // 使用 hashtable 作为底层机制
        typedef hashtable<ccystl::pair<const Key, T>, Hash, KeyEqual> base_type;
        base_type ht_;
    ...
    public:
        ...
        unordered_map(InputIterator first, InputIterator last,
            ...
            const Hash& hash = Hash(),
            const KeyEqual& equal = KeyEqual())
            : ht_(ccystl::max(bucket_count, static_cast<size_type>(ccystl::distance(first, last))), hash, equal) {
        ...
        unordered_map(std::initializer_list<value_type> ilist,
            ...
            const Hash& hash = Hash(),
            const KeyEqual& equal = KeyEqual())
            :ht_(ccystl::max(bucket_count, static_cast<size_type>(ilist.size())), hash, equal) {
        ...
        pair<iterator, bool> emplace(Args&& ...args) {
            return ht_.emplace_unique(ccystl::forward<Args>(args)...);
        }

        template <class ...Args>
        iterator emplace_hint(const_iterator hint, Args&& ...args) {
            return ht_.emplace_unique_use_hint(hint, ccystl::forward<Args>(args)...);
        }
        ...
        pair<iterator, bool> insert(value_type&& value) {
            return ht_.emplace_unique(ccystl::move(value));
        }
        ...
        iterator insert(const_iterator hint, value_type&& value) {
            return ht_.emplace_unique_use_hint(hint, ccystl::move(value));
        }
        ...
        mapped_type& operator[](key_type&& key) {
            iterator it = ht_.find(key);
            if (it.node == nullptr)
                it = ht_.emplace_unique(ccystl::move(key), T{}).first;
            return it->second;
        }
