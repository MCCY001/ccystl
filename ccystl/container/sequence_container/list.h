#ifndef CCYSTL_LIST_H_
#define CCYSTL_LIST_H_
...
//
// 异常保证：
// ccystl::list<T> 满足基本异常保证，部分函数无异常保证，并对以下等函数做强异常安全保证：
//   * emplace_front
//   * emplace_back
//   * emplace
//   * push_front
...
// list 的迭代器设计
template <class T>
struct list_iterator : public ccystl::iterator<ccystl::bidirectional_iterator_tag, T> {
...
public:
    ...
    typedef ccystl::allocator<list_node_base<T>> base_allocator;
    typedef ccystl::allocator<list_node<T>> node_allocator;
...
public:
    ...
    void emplace_front(Args&&... args) {
        ...
        auto link_node = create_node(ccystl::forward<Args>(args)...);
    ...
    iterator insert(const_iterator pos, value_type&& value) {
        THROW_LENGTH_ERROR_IF(size_ > max_size() - 1, "list<T>'s size too big");
        auto link_node = create_node(ccystl::move(value));
        ++size_;
        return link_iter_node(pos, link_node->as_base());
    }
    ...
    void sort() {
        list_sort(begin(), end(), size(), ccystl::less<T>());
    }
...
void list<T>::reverse() {
    ...
    while (i.node_ != e.node_) {
        ccystl::swap(i.node_->prev, i.node_->next);
        i.node_ = i.node_->prev;
    }
    ccystl::swap(e.node_->prev, e.node_->next);
}
...
list<T>::create_node(Args&&... args) {
    ...
    try {
        data_allocator::construct(ccystl::address_of(p->value), ccystl::forward<Args>(args)...);
        p->prev = nullptr;
        p->next = nullptr;
    }
    catch (...) {
...
void list<T>::destroy_node(node_ptr p) {
    data_allocator::destroy(ccystl::address_of(p->value));
    node_allocator::deallocate(p);
}
...
template <class T>
bool operator<(const list<T>& lhs, const list<T>& rhs) {
    return ccystl::lexicographical_compare(lhs.cbegin(), lhs.cend(), rhs.cbegin(), rhs.cend());
}
