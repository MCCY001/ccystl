#ifndef CCYSTL_DEQUE_H_
#define CCYSTL_DEQUE_H_
...
//
// 异常保证：
// ccystl::deque<T> 满足基本异常保证，部分函数无异常保证，并对以下等函数做强异常安全保证：
//   * emplace_front
//   * emplace_back
//   * emplace
//   * push_front
...
#define DEQUE_MAP_INIT_SIZE 8
    ...
    public:
        // deque 的型别定义
        typedef ccystl::allocator<T>                      allocator_type;
        typedef ccystl::allocator<T>                      data_allocator;
        typedef ccystl::allocator<T*>                     map_allocator;
        ...
        typedef deque_iterator<T, T&, T*>                iterator;
        typedef deque_iterator<T, const T&, const T*>    const_iterator;
        typedef ccystl::reverse_iterator<iterator>        reverse_iterator;
        typedef ccystl::reverse_iterator<const_iterator>  const_reverse_iterator;
    ...
    public:
        ...
        template <class IIter, typename std::enable_if<
            ccystl::is_input_iterator<IIter>::value, int>::type = 0>
        deque(IIter first, IIter last) {
            copy_init(first, last, iterator_category(first));
        }

        deque(std::initializer_list<value_type> ilist) {
            copy_init(ilist.begin(), ilist.end(), ccystl::forward_iterator_tag());
        }

        deque(const deque& rhs) {
            copy_init(rhs.begin(), rhs.end(), ccystl::forward_iterator_tag());
        }
        deque(deque&& rhs) noexcept
            :begin_(ccystl::move(rhs.begin_)),
            end_(ccystl::move(rhs.end_)),
            map_(rhs.map_),
    ...
    public:
        ...
        void     assign(std::initializer_list<value_type> ilist) {
            copy_assign(ilist.begin(), ilist.end(), ccystl::forward_iterator_tag{});
        }
        ...
        void     push_front(value_type&& value) { emplace_front(ccystl::move(value)); }
        void     push_back(value_type&& value) { emplace_back(ccystl::move(value)); }
    ...
    deque<T>& deque<T>::operator=(const deque& rhs) {
        if (this != &rhs) {
            const auto len = size();
            if (len >= rhs.size()) {
                erase(ccystl::copy(rhs.begin_, rhs.end_, begin_), end_);
            }
            else {
                iterator mid = rhs.begin() + static_cast<difference_type>(len);
                ccystl::copy(rhs.begin_, mid, begin_);
    ...
    void deque<T>::emplace_front(Args&& ...args) {
        if (begin_.cur != begin_.first) {
            data_allocator::construct(begin_.cur - 1, ccystl::forward<Args>(args)...);
            --begin_.cur;
        }
        else {
            ...
            try {
                --begin_;
                data_allocator::construct(begin_.cur, ccystl::forward<Args>(args)...);
            }
            catch (...) {
                ++begin_;
                throw;
    ...
    void deque<T>::emplace_back(Args&& ...args) {
        if (end_.cur != end_.last - 1) {
            data_allocator::construct(end_.cur, ccystl::forward<Args>(args)...);
            ++end_.cur;
        }
        else {
    ...
    typename deque<T>::iterator deque<T>::emplace(iterator pos, Args&& ...args) {
        if (pos.cur == begin_.cur) {
            emplace_front(ccystl::forward<Args>(args)...);
            return begin_;
        }
        else if (pos.cur == end_.cur) {
            emplace_back(ccystl::forward<Args>(args)...);
            return end_ - 1;
        }
        return insert_aux(pos, ccystl::forward<Args>(args)...);
    }
    ...
    typename deque<T>::iterator
        ...
        if (position.cur == begin_.cur) {
            emplace_front(ccystl::move(value));
            return begin_;
        }
        else if (position.cur == end_.cur) {
            emplace_back(ccystl::move(value));
            auto tmp = end_;
            --tmp;
            return tmp;
        }
        else {
            return insert_aux(position, ccystl::move(value));
        }
    }
    ...
    void deque<T>::insert(iterator position, size_type n, const value_type& value) {
        if (position.cur == begin_.cur) {
            ...
            ccystl::uninitialized_fill_n(new_begin, n, value);
            begin_ = new_begin;
        }
        else if (position.cur == end_.cur) {
            require_capacity(n, false);
            auto new_end = end_ + n;
            ccystl::uninitialized_fill_n(end_, n, value);
            end_ = new_end;
        }
        else {
    ...
    typename deque<T>::iterator
        ...
        if (elems_before < (size() / 2)) {
            ccystl::copy_backward(begin_, position, next);
            pop_front();
        }
        else {
            ccystl::copy(next, end_, position);
            pop_back();
        }
    ...
    void deque<T>::clear() {
        ...
        if (begin_.node != end_.node) { // 有两个以上的缓冲区
            ccystl::destroy(begin_.cur, begin_.last);
            ccystl::destroy(end_.first, end_.cur);
        }
        else {
            ccystl::destroy(begin_.cur, end_.cur);
        }
        shrink_to_fit();
        end_ = begin_;
    }
    ...
    void deque<T>::swap(deque& rhs) noexcept {
        if (this != &rhs) {
            ccystl::swap(begin_, rhs.begin_);
            ccystl::swap(end_, rhs.end_);
            ccystl::swap(map_, rhs.map_);
            ccystl::swap(map_size_, rhs.map_size_);
        }
    }
    ...
    void deque<T>::
        ...
        map_size_ = ccystl::max(static_cast<size_type>(DEQUE_MAP_INIT_SIZE), nNode + 2);
        try {
    ...
    void deque<T>::
        ...
        if (n != 0) {
            for (auto cur = begin_.node; cur < end_.node; ++cur) {
                ccystl::uninitialized_fill(*cur, *cur + buffer_size, value);
            }
            ccystl::uninitialized_fill(end_.first, end_.cur, value);
        }
    }
    ...
    void deque<T>::
        ...
        const size_type n = ccystl::distance(first, last);
        map_init(n);
    ...
    void deque<T>::
        ...
        for (auto cur = begin_.node; cur < end_.node; ++cur) {
            auto next = first;
            ccystl::advance(next, buffer_size);
            ccystl::uninitialized_copy(first, next, *cur);
            first = next;
        }
        ccystl::uninitialized_copy(first, last, end_.first);
    }
    ...
    void deque<T>::
        ...
        const size_type len2 = ccystl::distance(first, last);
        if (len1 < len2) {
            auto next = first;
            ccystl::advance(next, len1);
            ccystl::copy(first, next, begin_);
            insert_dispatch(end_, next, last, forward_iterator_tag{});
        }
        else {
            erase(ccystl::copy(first, last, begin_), end_);
        }
    }
    ...
    typename deque<T>::iterator
        ...
        const size_type elems_before = position - begin_;
        value_type value_copy = value_type(ccystl::forward<Args>(args)...);
        if (elems_before < (size() / 2)) { // 在前半段插入
            ...
            --back2;
            position = begin_ + elems_before;
            ccystl::copy_backward(position, back2, back1);
        }
        *position = ccystl::move(value_copy);
        return position;
    }
        ...
        if (elems_before < (len / 2)) {
            ...
            try {
                if (elems_before >= n) {
                    ...
                    ccystl::uninitialized_copy(begin_, begin_n, new_begin);
                    begin_ = new_begin;
                    ccystl::copy(begin_n, position, old_begin);
                    ccystl::fill(position - n, position, value_copy);
                }
                else {
                    ccystl::uninitialized_fill(
                        ccystl::uninitialized_copy(begin_, position, new_begin), begin_, value_copy);
                    begin_ = new_begin;
                    ccystl::fill(old_begin, position, value_copy);
                }
            }
            catch (...) {
            ...
            try {
                if (elems_after > n) {
                    auto end_n = end_ - n;
                    ccystl::uninitialized_copy(end_n, end_, end_);
                    end_ = new_end;
                    ccystl::copy_backward(position, end_n, old_end);
                    ccystl::fill(position, position + n, value_copy);
                }
                else {
                    ccystl::uninitialized_fill(end_, position + n, value_copy);
                    ccystl::uninitialized_copy(position, end_, position + n);
                    end_ = new_end;
                    ccystl::fill(position, old_end, value_copy);
                }
            }
            catch (...) {
        ...
        if (elems_before < (len / 2)) {
            ...
            try {
                if (elems_before >= n) {
                    ...
                    begin_ = new_begin;
                    ccystl::copy(begin_n, position, old_begin);
                    ccystl::copy(first, last, position - n);
                }
                else {
                    auto mid = first;
                    ccystl::advance(mid, n - elems_before);
                    ccystl::uninitialized_copy(first, mid,
                        ccystl::uninitialized_copy(begin_, position, new_begin));
                    begin_ = new_begin;
                    ccystl::copy(mid, last, old_begin);
                }
            }
            catch (...) {
                if (new_begin.node != begin_.node)
            ...
            try {
                if (elems_after > n) {
                    ...
                    end_ = new_end;
                    ccystl::copy_backward(position, end_n, old_end);
                    ccystl::copy(first, last, position);
                }
                else {
                    auto mid = first;
                    ccystl::advance(mid, elems_after);
                    ccystl::uninitialized_copy(position, end_,
                        ccystl::uninitialized_copy(mid, last, end_));
                    end_ = new_end;
                    ccystl::copy(first, mid, position);
                }
            }
            catch (...) {
                if (new_end.node != end_.node)
    ...
    void deque<T>::
        ...
        if (position.cur == begin_.cur) {
            ...
            auto new_begin = begin_ - n;
            try {
                ccystl::uninitialized_copy(first, last, new_begin);
                begin_ = new_begin;
            }
            catch (...) {
                if (new_begin.node != begin_.node)
            ...
            auto new_end = end_ + n;
            try {
                ccystl::uninitialized_copy(first, last, end_);
                end_ = new_end;
            }
            catch (...) {
                if (new_end.node != end_.node)
    ...
    void deque<T>::reallocate_map_at_front(size_type need_buffer) {
        const size_type new_map_size = ccystl::max(map_size_ << 1,
    ...
    bool operator==(const deque<T>& lhs, const deque<T>& rhs) {
        return lhs.size() == rhs.size() &&
            ccystl::equal(lhs.begin(), lhs.end(), rhs.begin());
    }

    template <class T>
    bool operator<(const deque<T>& lhs, const deque<T>& rhs) {
        return ccystl::lexicographical_compare(
