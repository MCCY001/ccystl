...
OutputIter unchecked_copy_cat(InputIter first, InputIter last,
                              OutputIter result, ccystl::input_iterator_tag) {
...
std::enable_if_t<std::is_same_v<std::remove_const_t<Tp>, Up> &&
                 std::is_trivially_copy_assignable_v<Up>,
                 Up*>
...
ccystl::pair<InputIter, OutputIter>
...
ccystl::pair<RandomIter, OutputIter>
unchecked_copy_n(RandomIter first, Size n, OutputIter result,
                 ccystl::random_access_iterator_tag) {
    auto last = first + n;
    return ccystl::pair<RandomIter, OutputIter>(
        last, ccystl::copy(first, last, result));
}

template <class InputIter, class Size, class OutputIter>
ccystl::pair<InputIter, OutputIter> copy_n(InputIter first, Size n,
                                           OutputIter result) {
...
template <class Tp, class Up>
std::enable_if_t<std::is_same_v<std::remove_const_t<Tp>, Up> &&
                 std::is_trivially_move_assignable_v<Up>,
                 Up*>
...
unchecked_move_backward_cat(BidirectionalIter1 first, BidirectionalIter1 last,
                            BidirectionalIter2 result,
                            ccystl::bidirectional_iterator_tag) {
    while (first != last)
...
// 为 one-byte 类型提供特化版本(一般是char类型)
template <class Tp, class Size, class Up>
std::enable_if_t<std::is_integral_v<Tp> && sizeof(Tp) == 1 &&
                 !std::is_same_v<Tp, bool> && std::is_integral_v<Up> &&
                 sizeof(Up) == 1,
                 Tp*>
