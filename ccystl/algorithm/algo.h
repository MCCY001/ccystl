...
find_end_dispatch(BidirectionalIter1 first1, BidirectionalIter1 last1,
    ...
    reviter1 rresult =
        ccystl::search(reviter1(last1), rlast1, reviter2(last2), rlast2);
    if (rresult == rlast1) {
        return last1;
    }
...
ForwardIter1 find_end(ForwardIter1 first1, ForwardIter1 last1,
    ...
    return ccystl::find_end_dispatch(first1, last1, first2, last2, Category1(),
                                     Category2());
}
...
find_end_dispatch(BidirectionalIter1 first1, BidirectionalIter1 last1,
    ...
    reviter2 rlast2(first2);
    reviter1 rresult =
        ccystl::search(reviter1(last1), rlast1, reviter2(last2), rlast2, comp);
    if (rresult == rlast1) {
        return last1;
    }
    else {
...
template <class RandomIter, class T>
pair<RandomIter, RandomIter> erange_dispatch(RandomIter first, RandomIter last,
                                             const T& value,
                                             random_access_iterator_tag) {
    auto len = last - first;
    auto half = len;
...
ForwardIter remove(ForwardIter first, ForwardIter last, const T& value) {
    ...
    return first == last
               ? first
               : ccystl::remove_copy(++next, last, first, value);
}
...
ForwardIter remove_if(ForwardIter first, ForwardIter last,
                      UnaryPredicate unary_pred) {
    first = ccystl::find_if(first, last,
                            unary_pred); // 利用 find_if 找出第一个匹配的地方
    auto next = first;
    return first == last
               ? first
               : ccystl::remove_copy_if(++next, last, first, unary_pred);
}
...
/**
 * @brief 随机打乱范围[first, last)内的元素顺序。
 *
 * 该函数接受两个迭代器，表示待打乱的元素范围。使用 C++11
 * 随机库确保生成高质量的随机数。 如果范围为空（即 first ==
 * last），该函数不会执行任何操作。
 *
...
void random_shuffle(RandomIter first, RandomIter last) {
    ...
    std::mt19937 eng(rd()); // 初始化 Mersenne Twister 引擎
    std::shuffle(first, last, eng); // 使用 std::shuffle
}
...
bool is_permutation_aux(ForwardIter1 first1, ForwardIter1 last1,
    ...
    constexpr bool is_ra_it =
        ccystl::is_random_access_iterator<ForwardIter1>::value &&
...
void merge_without_buffer(BidirectionalIter first, BidirectionalIter middle,
    ...
    ccystl::merge_without_buffer(new_middle, second_cut, last, len1 - len11,
                                 len2 - len22);
}
...
void merge_adaptive(BidirectionalIter first, BidirectionalIter middle,
    ...
    if (len1 <= len2 && len1 <= buffer_size) {
        ...
        auto new_middle =
            ccystl::rotate_adaptive(first_cut, middle, second_cut, len1 - len11,
                                    len22, buffer, buffer_size);
        ccystl::merge_adaptive(first, first_cut, new_middle, len11, len22, buffer,
                               buffer_size);
...
void inplace_merge_aux(BidirectionalIter first, BidirectionalIter middle,
    ...
    if (!buf.begin()) {
        ...
        ccystl::merge_adaptive(first, middle, last, len1, len2, buf.begin(),
                               buf.size());
    }
}
...
// 的输出区间 其余放到 result_false 的输出区间，并返回一个 ccystl::pair
// 指向这两个区间的尾部
/*****************************************************************************************/
...
void intro_sort(RandomIter first, RandomIter last, Size depth_limit) {
    while (static_cast<size_t>(last - first) > kSmallSectionSize) {
        ...
        --depth_limit;
        auto mid =
            ccystl::median(*(first), *(first + (last - first) / 2), *(last - 1));
...
void nth_element(RandomIter first, RandomIter nth, RandomIter last) {
    if (nth == last)
        return;
    while (last - first > 3) {
        auto cut = ccystl::unchecked_partition(
            first, last,
            ccystl::median(*first, *(first + (last - first) / 2), *(last - 1)));
