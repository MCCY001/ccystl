...
namespace ccystl {
    ...
    void push_heap_d(RandomIter first, RandomIter last, Distance*) {
        ccystl::push_heap_aux(first, (last - first) - 1, static_cast<Distance>(0), *(last - 1));
    }
    ...
    void adjust_heap(RandomIter first, Distance holeIndex, Distance len, T value) {
        ...
        ccystl::push_heap_aux(first, holeIndex, topIndex, value);
    }
    ...
    void pop_heap(RandomIter first, RandomIter last) {
        ccystl::pop_heap_aux(first, last - 1, last - 1, *(last - 1), distance_type(first));
    }
    ...
    void adjust_heap(RandomIter first, Distance holeIndex, Distance len, T value,
        ...
        // 再执行一次上溯(percolate up)过程
        ccystl::push_heap_aux(first, holeIndex, topIndex, value, comp);
    }
    ...
    void pop_heap_aux(RandomIter first, RandomIter last, RandomIter result,
        ...
        *result = *first;  // 先将尾指设置成首值，即尾指为欲求结果
        ccystl::adjust_heap(first, static_cast<Distance>(0), last - first, value, comp);
    }
    ...
    void sort_heap(RandomIter first, RandomIter last, Compared comp) {
        while (last - first > 1) {
            ccystl::pop_heap(first, last--, comp);
        }
    }
    ...
    void make_heap_aux(RandomIter first, RandomIter last, Distance*, Compared comp) {
        ...
        while (true) {
            // 重排以 holeIndex 为首的子树
            ccystl::adjust_heap(first, holeIndex, len, *(first + holeIndex), comp);
            if (holeIndex == 0)
                return;
            holeIndex--;
        }
